  ...
  if (token == null) {
    ...
    document.getElementById("loged-in-profile").innerHTML = `
       <a class="navbar-brand " style="cursor:pointer" onclick="sendUserIdLogedIn()">
                ...
                <img src=${typeof user.profile_image == "object" ? ' "./images/default_profile_image.png"' : user.profile_image} class="rounded-circle border border-2"style="width: 40px; height:40px ;" alt="profile">
        </a>
      `
  }
}
...
function createPost(post){
  ...
  let user= JSON.parse(localStorage.getItem("user"));
  let token= localStorage.getItem("token")
  let profile_image= typeof  post.author.profile_image=="object" ? default_image :  post.author.profile_image;
   posts.innerHTML+=`
    <!--POST -->
     <div class="post ">
       <div class="card shadow-sm ">
         <div class="card-header">
           <a class="navbar-brand " style="cursor:pointer"  onclick='sendUserId(${post.author.id})'>
           ...
           <div class="${token!=null?(post.author.id==user.id ?"":"d-none"): "d-none"  } float-end ">
             <button  type="button" class="btn btn-secondary " onclick="editPostBtnClicked('${encodeURIComponent(JSON.stringify(post))}')">edit</button>
             <button type="button" class="btn btn-danger" onclick="deletePostBtnClicked('${encodeURIComponent(JSON.stringify(post))}')">delete</button>
           </div>
         </div>
         <div class="card-body p-0" style="cursor:pointer" onclick='sendPostId(${post.id})' >
         ...
         <div class="card-footer text-body-secondary">
           ...
           <span style=" cursor:pointer; " > (${post.comments_count}) comments</span>
